...
**Ownership.** Account for the asynchronous operations on the affected path: who observes failure, awaits completion, owns resources, and stops obsolete work? Honor the requested outcome and preserve settled choices outside the requested change.
...
When streams or response bodies are involved, let slow consumers apply backpressure. Use the existing stream abstractions and preserve their error and destruction semantics, especially after an HTTP response starts. Consume or cancel unused response bodies instead of relying on garbage collection.

For review, explain the affected ownership or sequencing issue without editing. When changing code, verify the relevant failure, interruption, saturation, or slow-consumer case with controlled coordination, bounded waits, and no unfinished work. Choose the case for the changed property, not every topic in this skill; report what remains unverified.
