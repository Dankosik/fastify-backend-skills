...
description: "Use to implement clear TypeScript backend requirements or an agreed design within existing project contracts."
---
...
**Reuse.** When a technical helper is needed, look for a semantic match in nearby code and APIs already available from the runtime, Fastify, or declared utilities. Keep the search proportional to the helper. Write custom mechanics only for a concrete gap; keep business policy explicit. A wrapper should add domain meaning or adaptation, not merely rename a library call.
