...
For a subset, replace `'*'` with names such as `typescript-implement`. For Claude standalone
...
```sh
gh skill install Dankosik/fastify-backend-skills typescript-implement --pin v1.0.0 --agent codex --scope project
...
claude plugin install fastify-backend-skills@dankosik-skills
```

Inside Claude Code, the same commands start with `/plugin`. Plugin skills have
a namespace, for example `/fastify-backend-skills:typescript-implement`. Refresh the catalog and
update this plugin when you choose to adopt a new release:

```sh
claude plugin marketplace update dankosik-skills
claude plugin update fastify-backend-skills@dankosik-skills
```
...
codex plugin add fastify-backend-skills@dankosik-skills
```
